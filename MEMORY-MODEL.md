...
- **working-set is per-session.** Each session writes its own `working-set.<id>.md`, keyed by the first hex chars of `CLAUDE_CODE_SESSION_ID` (the hook and the agent's `mem ws-path` derive the same path, so no coordination is needed). Two efforts never overwrite each other's "Now", even in the *same* checkout. It is volatile, not merged or shared; the default install git-ignores all of them. A resume reads the most recent with `mem ws-latest`; `mem ws-gc` prunes stale ones (keeping the latest and the current). Outside a session (id unset) it falls back to the legacy `working-set.md`.
...
Per-session working sets make two sessions in the *same* directory safe for the working set. The ledger and archive are still shared in-place files, though, so concurrent demotes/promotes from the same directory can race (last-write-wins on `ledger.md`). One git worktree per feature remains the clean pattern, git is the concurrency control for the shared tiers, and it also keeps those in-place edits from racing. See "Parallel development" in README.md.
